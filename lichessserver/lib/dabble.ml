...
let%expect_test "test simple heading" =
  ...
  | Ok h ->
    ...
    [%expect {| { Dabble.level = 1; text = ["hello"] } |}]
  | Error err ->
    Stdio.print_endline err;
    [%expect.unreachable]
;;
...
let%expect_test "test simple 2 word heading" =
  ...
  | Ok h ->
    Stdio.print_endline (show_header h);
    [%expect {| { Dabble.level = 1; text = ["hello"; "world"] } |}]
  | Error err ->
    Stdio.print_endline err;
    [%expect.unreachable]
;;
...
let%expect_test "test simple level 2 heading" =
  ...
  | Ok h ->
    Stdio.print_endline (show_header h);
    [%expect {| { Dabble.level = 2; text = ["hello"; "world"] } |}]
  | Error err ->
    Stdio.print_endline err;
    [%expect.unreachable]
;;
...
let%expect_test "some whole file" =
  ...
  | Ok lines ->
    ...
    [%expect
      {|
      (Dabble.Heading { Dabble.level = 2; text = ["this"; "is"; "a"; "heading"] })
      (Dabble.Text [])
      (Dabble.ListItem ["some"; "list"; "item"])
      (Dabble.ListItem ["some"; "list"; "item"])
      (Dabble.Text [])
      (Dabble.Text ["text"; "inbetween"; "the"; "list"; "items"])
      (Dabble.Text [])
      (Dabble.ListItem ["some"; "list"; "item"])
      (Dabble.ListItem ["some"; "list"; "item"; "2"])
      (Dabble.Text [])
      (Dabble.Heading { Dabble.level = 4; text = ["heading"; "in"; "the"; "wild"] })
      (Dabble.Text [])
      (Dabble.Text ["text"; "that"; "goes"; "over"; "very"; "two"; "lines"])
      (Dabble.Text ["of"; "text"; "that"; "goes"; "over"])
      (Dabble.Text [])
      (Dabble.Text ["goes"; "over"; "two"; "lines"])
      |}]
  | Error err ->
    Stdio.print_endline err;
    [%expect.unreachable]
...
let%expect_test "some other example" =
  ...
  | Ok lines ->
    ...
    [%expect
      {|
      (Dabble.Heading
         { Dabble.level = 1; text = ["This"; "is"; "a"; "simple"; "paragraph"] })
      (Dabble.Text [])
      (Dabble.Text
...
let json =
  fix (fun json ->
    ...
    let list =
      ws *> char '[' *> lex (sep_by (char ',' <* ws) value)
      <* char ']'
      >>| fun l -> JList l
    in
    let key_value_pair =
...
let%expect_test "simple json parsing" =
  ...
  | Ok r ->
    ...
    [%expect
      {|
      key: name; value: (Dabble.JString "example")
      key: count; value: (Dabble.JNumber 42)
      key: nested; value: (Dabble.JObject [("a", (Dabble.JString "b"))])
      key: list; value: (Dabble.JList
         [(Dabble.JNumber 1); (Dabble.JNumber 2); (Dabble.JNumber 3);
           (Dabble.JNumber 4)])
      |}]
  | Error err ->
    Stdio.print_endline err;
